...
class Users {
  ...
  static async storeUser(conn, user) {
    let sql_delete = 'DELETE radcheck, radusergroup FROM radcheck INNER JOIN radusergroup WHERE radcheck.username = radusergroup.username AND radcheck.username = ?;';

    let sql_insert = 'BEGIN;' +
  ...
  static async deleteUser(conn, {
    ...
    let sql_delete = 'DELETE radcheck, radusergroup ' +
...
const get_ip4Pools = async function (conn) {
  const sql = 'SELECT distinct pool_name ' +
		'FROM radippool ' +
