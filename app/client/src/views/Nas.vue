<template>
  <v-card class="primary" style="min-width: 100%">
    <v-layout>
      <v-flex>
        <v-dialog v-model="nasDialog" max-width="800px" dark>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" color="accent" class="mb-2">New NAS</v-btn>
          </template>
          <v-card color="secondary" dark>
            <v-card-title class="headline primary accent--text" primary-title>{{
              editing ? 'Edit NAS' : 'New NAS'
            }}</v-card-title>
    ...
    <v-data-table
      ...
      :server-items-length="getNaslist.count"
